...
%
% \begin{itemize}
% \item<1-> Our discussion on Fourier inversion theorem shows that, given a sequence of probability measure $\{\mu_n\}$ whose characteristic function convergences to the characteristic function of another measure $\mu$, if a subsequence of $\{\mu_n\}$ converges weakly to measure $\nu$, then $\mu=\nu$. 
% 
% \item<2-> To resolve the issue on whether such subsequence of $\{\mu_n\}$ exists, and whether  $\{\mu_n\}$ itself would converge weakly to the limit of its subsequence, we need to introduce the concept of tightness. 
% 
